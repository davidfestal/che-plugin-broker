...
build-artifacts:
	$(GOENV) go build $(GOFLAGS) -o artifacts-plugin-broker brokers/artifacts/cmd/main.go

.PHONY: build-metadata
build-metadata:
	$(GOENV) go build $(GOFLAGS) -o metadata-plugin-broker brokers/metadata/cmd/main.go
...
build-docker-artifacts:
	docker build -t eclipse/che-artifacts-plugin-broker:latest -f build/artifacts/Dockerfile .

.PHONY: build-docker-metadata
build-docker-metadata:
	docker build -t eclipse/che-metadata-plugin-broker:latest -f build/metadata/Dockerfile .
