...
docker build -t eclipse/che-artifacts-plugin-broker:${IMAGE_TAG} -f ${ROOT_DIR}/build/artifacts/Dockerfile ${ROOT_DIR}
docker push eclipse/che-artifacts-plugin-broker:${IMAGE_TAG}
# Push latest bugfix image
docker tag eclipse/che-artifacts-plugin-broker:${IMAGE_TAG} eclipse/che-artifacts-plugin-broker:${IMAGE_LATEST_TAG}
docker push eclipse/che-artifacts-plugin-broker:${IMAGE_LATEST_TAG}

# Metadata Plugin Broker
docker build -t eclipse/che-metadata-plugin-broker:${IMAGE_TAG} -f ${ROOT_DIR}/build/metadata/Dockerfile ${ROOT_DIR}
docker push eclipse/che-metadata-plugin-broker:${IMAGE_TAG}
# Push latest bugfix image
docker tag eclipse/che-metadata-plugin-broker:${IMAGE_TAG} eclipse/che-metadata-plugin-broker:${IMAGE_LATEST_TAG}
docker push eclipse/che-metadata-plugin-broker:${IMAGE_LATEST_TAG}
