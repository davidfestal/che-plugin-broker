...
const errorMutuallyExclusiveTemplate = "VS Code extension description of the plugin '%s:%s' contains more than one mutually exclusive field 'attributes.extension', 'url', 'extensions'"
const errorNoFieldsTemplate = "Neither 'extension' nor 'url' nor 'extensions' field found in VS Code extension description of the plugin '%s:%s'"
...
func (b *Broker) getExtensionsAndURLs(meta model.PluginMeta) (e []string, u []string, err error) {
	...
	if !isSet {
		return nil, nil, fmt.Errorf(errorNoFieldsTemplate, meta.ID, meta.Version)
	}
